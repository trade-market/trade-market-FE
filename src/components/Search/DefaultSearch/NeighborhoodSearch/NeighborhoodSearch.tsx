...
const NeighborhoodSearch = () => {
  ...
  const rankingArr: rankingInterface = arr.reduce(
    (acc: any = {}, cur: string) => {
      acc[cur] = (acc[cur] || 0) + 1;
      return acc;
    },
  ...
  const sorted = Object.entries(rankingArr).sort(
    ([_, b1]: number[], [__, b2]: number[]) => b2 - b1
  );
  const popularArr = sorted.map((v) => v[0]);

  const SearchClickHandler = (rank: string) => {
    navigate(`?searching=${rank}&type=물물`);
  };
        ...
        {popularArr.map((rank, i) => {
          return (
            <div
              key={i}
              className="item"
              onClick={() => SearchClickHandler(rank)}
            >
